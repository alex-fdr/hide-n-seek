...
export class TextCounter {
  constructor(props) {
    ...
    this.text = factory.textOld(this.getValue(value), `${fontSize}px gamefont`, color)
  ...
  updateValue(amount = 1) {
    if (this.bounceAnimation) {
      ...
      let tween = tweens.pulse(this.text, this.bounceFactor, this.bounceTime)
    ...
    let toValue = this.value + amount

    this.finalValue = toValue
    
    let dummy = { value: 0 }
    let tween = tweens.add(dummy, { value: amount }, this.changeTime, { easing: 'linear' })
